/*
 * SUMMARY:      Calculate the topographic wetness index
 * COMMENTS:
...
/*****************************************************************************
  ...
  Purpose      : Calculate the topographic index for the generation of wetness index fractional area curve.  
...
//  float corner_lat, corner_lon;
  float lat,lon,cent_lat,cent_long;
  ...
  if (argc != 8)
    {
      printf("Usage : %s <demfile> <units> <vertres> <afile> <bfile> <wifile> <fracfile>\n", argv[0]);
      ...
      printf("\t\tafile   : (output) arcinfo map of pixel upslope contributing areas\n");
      printf("\t\tbfile   : (output) arcinfo map of pixel slopes\n");
      printf("\t\twifile  : (output) arcinfo map of topographic wetness index values (= ln(a/b))\n");
			 ...
			 exit(1); }
			   ...
			   for (k = count-1; k >-1; k--) 
			     { 
			       ...
			       cent_lat = yorig + rows*size - y*size-size/2;/*centre latitude */
...
//	  printf("wetness=%f area = %f contrib area %.4f\n",wetnessindex[y][x],a_pixel[y][x],a[y][x]);
  ...
  for (counter = 0; counter < num_bins; counter++)
      ...
      for(i=0; i<rows;i++) 
	{
	  for(j=0; j<cols; j++)
	    {
	      if(dem[i][j]!=nodata && wetnessindex[i][j] != nodata)
		{
...
***********************************************************************/
  ...
  dlat  = size / 10.;
  ddist = get_dist(cent_lat,cent_long,cent_lat+dlat,cent_long);

  areasum  = 0.;

  tmpsum = 0.0;
  tmplat = cent_lat-0.5*size + 0.5*dlat;
  for(i=0;i<10;i++) {
    tmpsum += get_dist(tmplat,cent_long-0.5*size,tmplat,cent_long+0.5*size) * ddist;
    tmplat += dlat;
  }
  areasum = tmpsum;
